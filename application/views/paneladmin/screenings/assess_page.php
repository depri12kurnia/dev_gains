                ...
                <div class="card card-primary shadow-sm mb-0 d-flex flex-column flex-fill" style="height: 100%;">
                    ...
                    <div class="card-header" style="flex-shrink: 0;">
                        <h3 class="card-title font-weight-bold text-sm">
                            <i class="fas fa-calculator mr-2"></i> Screening Form Select a score (1-5) to view its specific descriptor.
                        </h3>
                    </div>

                    <form action="<?= base_url('admin/screenings/store'); ?>" method="POST" class="d-flex flex-column flex-fill mb-0" style="height: calc(100% - 40px); overflow: hidden;">
                            ...
                            <div class="card card-outline card-secondary mt-4 shadow-sm">
                                <div class="card-header bg-light">
                                    <h3 class="card-title text-sm font-weight-bold text-secondary">
                                        <i class="fas fa-comments mr-1"></i> Reviewer's Notes Section
                                    </h3>
                                </div>
                                <div class="card-body">
                                    <div class="form-group mb-3">
                                        <label class="font-weight-bold text-xs text-dark uppercase">Notes the Submission *</label>
                                        <textarea name="notes" placeholder="Write Notes" class="form-control text-sm" rows="2" required><?= isset($submission->notes) ? htmlspecialchars($submission->notes) : '' ?></textarea>
                                    </div>

                                </div>
                            </div>

                            <!-- Screening Recommendation Status Options -->
                            <div class="card card-outline card-secondary mt-3 shadow-sm">
                                <div class="card-header bg-light">
                                    <h3 class="card-title text-sm font-weight-bold text-secondary">
                                        <i class="fas fa-gavel mr-1"></i> Screening Recommendation
                                    </h3>
                                </div>
                                <div class="card-body p-3">
                                    <?php $saved_status = isset($submission->screening_status) ? $submission->screening_status : ''; ?>
                                    <div class="d-flex flex-column flex-md-row justify-content-start" style="gap: 20px;">

                                        <div class="custom-control custom-radio">
                                            <input class="custom-control-input" type="radio" id="status_final" name="screening_status" value="Qualified" <?= $saved_status == 'Qualified' ? 'checked' : '' ?> required>
                                        ...
                                        <div class="custom-control custom-radio">
                                            <input class="custom-control-input" type="radio" id="status_minor" name="screening_status" value="Not Qualified" <?= $saved_status == 'Not Qualified' ? 'checked' : '' ?>>
                        ...
                        <div class="card-footer bg-white d-flex justify-content-end align-items-center" style="flex-shrink: 0; gap: 15px; border-top: 1px solid #efefef;">
                            <div class="text-md">
                                <span class="font-weight-bold text-muted mr-2">Total Score Screening:</span>
                            ...
                            <button type="submit" class="btn btn-success btn-sm px-4 font-weight-bold shadow-sm">
                                <i class="fas fa-save mr-1"></i> Save Score Screening
                            </button>
                        </div>
                    </form>
                </div>
