...
class Auth extends CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			// Cek apakah user melebihi batas login attempts
			if ($this->ion_auth->is_max_login_attempts_exceeded($identity)) {
				$this->session->set_flashdata('message', 'Terlalu banyak percobaan login. Anda Di Blokir Selama 5 Menit Silakan coba lagi nanti.');
				redirect('auth/login', 'refresh');
			}
	...
	public function forgot_password_process()
	{
		...
		if ($forgotten) {
			$this->session->set_flashdata('message', 'Silakan cek email Anda untuk mereset password.');
			redirect('auth/login');
		} else {
			$this->session->set_flashdata('message', 'Gagal mengirim email reset password!');
			redirect('auth/forgot_password');
		}
	}
}
