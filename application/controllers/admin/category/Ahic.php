...
class Ahic extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('M_settings');
        $this->load->model('M_r_ahic');
        $this->load->model('M_log_user');
        $this->load->model('M_users');

        if (!$this->ion_auth->in_group(array('admin', 'ahic'))) {
    ...
    public function index()
    {
        ...
        $data['title'] = 'Assessment AHIC Management | Admin Panel';
        $data['content'] = 'paneladmin/r_ahic/list';
    ...
    public function ajax_list()
    {
        ...
        $list = $this->M_r_ahic->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        foreach ($list as $submission) {
            ...
            if ($my_evaluation) {
                $row[] = '<div class="btn-group" style="gap:4px;">
                        ...
                        <a class="btn btn-warning btn-sm font-weight-bold shadow-sm text-dark" href="' . base_url('admin/category/ahic/assess/' . $submission->id) . '" target="_blank"><i class="fa fa-edit"></i> Edit Score</a>
                      </div>';
            } else {
                $row[] = '<div class="btn-group" style="gap:4px;">
                        <a class="btn btn-info btn-sm shadow-sm" href="javascript:void(0)" title="View Info" onclick="viewSubmission(' . "'" . $submission->id . "'" . ')"><i class="fa fa-eye"></i>View Details</a>
                        <a class="btn btn-primary btn-sm font-weight-bold shadow-sm" href="' . base_url('admin/category/ahic/assess/' . $submission->id) . '" target="_blank"><i class="fa fa-calculator"></i> Assess Now</a>
                      </div>';
            }
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_r_ahic->count_all(),
            "recordsFiltered" => $this->M_r_ahic->count_filtered(),
            "data" => $data,
    ...
    {
        $data = $this->M_r_ahic->get_by_id($id);
    ...
    {
        $submission = $this->M_r_ahic->get_by_id($submission_id);
        if (!$submission) {
            show_404();
        }
        ...
        $data['components'] = $this->M_r_ahic->get_components_by_category($category);
        ...
        $existing_details = $evaluation ? $this->M_r_ahic->get_evaluation_details($evaluation->id) : [];
    ...
    public function store()
    {
        ...
        $process = $this->M_r_ahic->save_evaluation($this->input->post(), $user->id);
